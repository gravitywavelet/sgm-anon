...
def ensure_tables(conn):
    ...
    # runs table (shared schema with trainer)
    _ensure_runs_table(conn)

    conn.commit()

def log_run_row(db_path, exp_name, seed, steps, mean_reward):
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    conn = sqlite3.connect(db_path, timeout=60.0)
    _ensure_runs_table(conn)
    conn.execute("INSERT INTO runs(ts,exp_name,seed,steps,mean_reward) VALUES(?,?,?,?,?)",
                 (int(time.time()), exp_name, int(seed), int(steps), float(mean_reward)))
    conn.commit()
    conn.close()


def parse_args():
    p = argparse.ArgumentParser("Ex5 – CIFAR-100 single run (baseline/proposal)")
    # exp/meta
    p.add_argument("--exp_name", type=str, default="cifar100_baseline")
...
def make_loaders(data_dir, batch_size, augment, num_workers):
    ...
    train = torchvision.datasets.CIFAR100(root=data_dir, train=True, download=True, transform=train_tf)
    test  = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=True, transform=test_tf)
...
def _sqlite_type(v):
    # robust type guess (handles numpy scalars too)
    import numbers
    try:
        import numpy as np
...
def ensure_columns(conn, table, row_dict):
    ...
    # add any missing, with best-guess SQL types
    added = []
    for k, v in row_dict.items():
        if k not in have:
            coltype = _sqlite_type(v)
            conn.execute(f'ALTER TABLE {table} ADD COLUMN {k} {coltype}')
            added.append((k, coltype))
    if added:
        conn.commit()
        print(f"[auto-migrate] Added columns on '{table}': {added}")
...
def main():
    ...
    # Log row (env_id uses “CIFAR100”; mean_reward stores accuracy)
    row = dict(
        ts=int(time.time()),
