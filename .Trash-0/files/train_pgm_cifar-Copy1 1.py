...
def parse_args():
    p = argparse.ArgumentParser("Ex4 – CIFAR-10 baseline/PGM train")
    # exp/meta
...
def make_loaders(data_dir, batch_size, augment, num_workers):
    ...
    test_loader  = DataLoader(test, batch_size=512, shuffle=False,
                              ...
                              persistent_workers=(num_workers//2>0), prefetch_factor=2 if num_workers//2>0 else None)
...
def evaluate(model, loader, device):
    ...
    for x,y in loader:
        ...
        tot  += y.numel()
    acc = 100.0 * corr / tot
    std = 0.0  # we’ll keep schema compatibility (std_reward) though not used here
    return acc, std
...
def main():
    ...
    # Log row (env_id is “CIFAR10” for consistency)
    row = dict(
        ts=int(time.time()),
