...
Outputs (in --out):
  ...
  - ex5_combined_from_sql.pdf         # 3-panel figure: μ/LB, decisions vs screen baselines, δ-spend
  - prints the iter=6 (confirm) row to stdout for easy copy into the paper
"""
...
def load_proposals(db_path, exp_name):
    ...
    df = pd.read_sql_query(
        "SELECT id, ts, exp_name, iter, proposer, accepted, delta, n_seeds, rmax, lb, mu_hat, incumbent_cfg, proposal_cfg "
    ...
    df["mu"] = df["mu_hat"]; df["delta_t"] = df["delta"]
...
def build_baselines(df):
    # index by (iter, stage) but we may still have multiple rows per key; we’ll pick the last
    ...
    def pick_last(indexed, it):
        if it not in indexed.index:
            return None
        obj = indexed.loc[it]
        # If multiple rows for this iter, take the last (they’re already ordered by ts ASC in load_proposals)
    ...
    for it in sorted(df["iter"].unique()):
        ...
        # Helpers to safely get scalar values
        def get(x, k):
            try:
                v = x[k]
                # If somehow still a Series, take last non-na
                if hasattr(v, "iloc"):
        ...
        # Baseline A: commit if screening LB > 0
        s_lb = None if s is None else get(s, "lb")
        baseA = "ACCEPT" if (s_lb is not None and pd.notna(s_lb) and float(s_lb) > 0.0) else "REJECT"

        # Baseline B: commit if screening μ >= 0.80
        s_mu = None if s is None else get(s, "mu")
        baseB = "ACCEPT" if (s_mu is not None and pd.notna(s_mu) and float(s_mu) >= 0.80) else "REJECT"
        ...
        rows.append({
            ...
            "delta_t": (None if s is None else get(s, "delta_t")) if s is not None else (None if c is None else get(c, "delta_t")),
            "SGM": sgm,
            "Screen_LB": baseA,
            "Screen_mu": baseB,
        })
...
def main(args):
    ...
    # Build baselines for the combined figure
    cmp_df = build_baselines(df)

    # Plot: combined 3-panel (ICLR-ready, two-column width)
    ...
    # (a) μ/LB across stages
    ax = axes[0]
    ax.plot(cmp_df["iter"], cmp_df["screen_mu"], marker="o", label=r"screen $\mu$")
    ax.plot(cmp_df["iter"], cmp_df["screen_lb"], marker="s", linestyle="--", label=r"screen LB")
    ax.plot(cmp_df["iter"], cmp_df["confirm_lb"], marker="^", linestyle="-.", label=r"confirm LB")
    if acc_iter is not None:
        y = cmp_df.loc[cmp_df["iter"]==acc_iter, "confirm_lb"]
        if not y.empty and pd.notna(y.iloc[0]): ax.scatter([acc_iter],[y.iloc[0]], marker="D", s=55, zorder=5, label="accepted")
    ax.set_xlabel("Iter"); ax.set_ylabel("Value"); ax.set_title("(a) μ/LB across stages")
    ax.legend(frameon=False, fontsize=7)

    # (b) Decisions: SGM vs screen-only baselines
    ax = axes[1]
    for name, col in [("SGM","SGM"), ("Screen-LB","Screen_LB"), ("Screen-μ","Screen_mu")]:
    ...
    ax.set_xlabel("Iter"); ax.set_title("(b) Commit decisions"); ax.legend(frameon=False, fontsize=7)
    ...
    if not d.empty:
        ...
        ax.legend(frameon=False, fontsize=7)
    ax.set_xlabel("Iter"); ax.set_ylabel("Alpha spend"); ax.set_title("(c) $\delta$-spending")

    plt.tight_layout()
    (out/"ex5_combined_from_sql.pdf").unlink(missing_ok=True)
    plt.savefig(out/"ex5_combined_from_sql.pdf", bbox_inches="tight"); plt.close()

    # Print the accepted row (iter 6) for quick copy
    if acc_iter is not None:
        r = df[(df["iter"]==acc_iter) & (df["stage"]=="confirm")].iloc[0]
        print("\n[Ex5: accepted confirm]")
        print(f" iter={acc_iter} | seeds={int(r['n_seeds'])} | lb={r['lb']:.4f} | mu={r['mu']:.4f}")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--db", required=True, help=".../runs/smoke_runs/pgm_c100_gate.db")
    ap.add_argument("--exp", required=True, help="cifar100_pgm_gate6")
