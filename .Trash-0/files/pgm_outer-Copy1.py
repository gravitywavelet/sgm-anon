#!/usr/bin/env python3
# Ex5 – PGM outer loop for CIFAR-100 (baseline/proposals + PAC gate)
...
def eb_lower_bound(y_list, delta, R=2.0):
    """
    y_list: list of paired diffs normalized to [-1,1] (Δ_pp / R, clipped)
    delta: per-iteration alpha spend δ_t
    R: sub-Gaussian range proxy (we use 2.0 percentage points before normalization)
...
# ---------- Proposal generator (1-knob random) ----------
def propose(cfg):
    """
    Randomly perturb one knob among: lr, weight_decay, batch_size, momentum.
    Keeps everything else fixed; returns (new_cfg, [changed_keys])
    """
    new = cfg.copy()
...
def launch_one(exp_name, seed, epochs, cfg, data_dir, runs_dir, db_name, num_workers):
    """
    Launches a single training run of ex5/train_pgm_cifar.py (CIFAR-100).
    """
...
def evaluate_config(conn, config_exp, seeds, epochs, cfg, label, data_dir, runs_dir,
    ...
    """
    Ensures (exp_name=config_exp, seed) results exist for the given cfg.
    Launches missing ones, returns:
      accs: dict seed -> test accuracy (%)
      times: dict seed -> wall-clock seconds for launched seeds
    """
    accs, to_run = {}, []
    for s in seeds:
...
def main():
    ...
    # Baseline (incumbent) – ensure/cache runs exist
    ...
    for it in range(1, args.budget+1):
        ...
        # Normalize paired improvements by rmax_pp and clip to [-1,1]
        y = []
        for s in seeds:
        ...
        lb, mu = eb_lower_bound(y, delta_t, R=1.0)
        ...
        if decision == "ACCEPT":
            ...
            inc_acc = prop_acc
            inc_mean = prop_mean
            inc_times = prop_times  # carry recent timings
