...
def make_seed_set(base_seeds, target_n):
    ...
    while len(base_seeds) + len(extra) < target_n:
        for s in base_seeds:
            ...
            if candidate not in used:
                ...
                used.add(candidate)
                if len(base_seeds) + len(extra) >= target_n:
                    break
        k += 1
...
ap.add_argument("--rmax_pp", type=float, default=2.0)
ap.add_argument("--confirm_mean_pp", type=float, default=1.5)
...
ap.add_argument(
    "--policy",
    choices=["sgm", "naive_screen", "best_screen"],
    default="sgm"
)
...
def propose(cfg, seed=None):
    """
    6-step preset (for early strong signal), then stochastic multi-knob proposals.
    Designed for long-horizon experiments (20–30 iterations).
    """
    global _PLAN_IDX
    base = deepcopy(cfg)
    _ensure_keys(base)

    # ---------- PRESET PHASE (first 6 iters) ----------
    if seed is not None and seed < len(_PRESET_PLAN):
        prop_cfg, changes = _apply_preset_changes(base, _PRESET_PLAN[seed])
        prop_cfg["_proposer_tag"] = "preset"
        return prop_cfg, changes
        

    # ---------- STOCHASTIC PHASE (after iter 6) ----------
    rng = random.Random(seed if seed is not None else int(time.time()))
    new = deepcopy(base)
    changes = []

    # ---- learning rate (small random walk) ----
    if rng.random() < 0.7:
        factor = rng.choice([0.85, 0.9, 1.1, 1.15])
        new["lr"] = float(new["lr"] * factor)
        changes.append(("lr@mul", factor))

    # ---- weight decay ----
    if rng.random() < 0.5:
        wd = rng.choice([0.03, 0.04, 0.05, 0.06, 0.08])
        new["weight_decay"] = wd
        changes.append(("weight_decay", wd))

    # ---- augmentation strength ----
    if rng.random() < 0.5:
        ra = rng.choice([7, 8, 9])
        new["randaug_m"] = ra
        changes.append(("randaug_m", ra))

    # ---- drop path ----
    if rng.random() < 0.4:
        dp = rng.choice([0.1, 0.15, 0.2])
        new["drop_path"] = dp
        changes.append(("drop_path", dp))

    # ---- mixup / cutmix toggle ----
    if rng.random() < 0.4:
        mix = rng.choice([0.0, 0.1, 0.4, 0.8])
        cut = 0.0 if mix < 0.2 else 1.0
        new["mixup"] = mix
        new["cutmix"] = cut
        changes.append(("mixup", mix))
        changes.append(("cutmix", cut))

    # ---- label smoothing ----
    if rng.random() < 0.3:
        ls = rng.choice([0.0, 0.05, 0.1])
        new["label_smoothing"] = ls
        changes.append(("label_smoothing", ls))

    # always keep EMA off (consistent with your finding)
    new["ema"] = False

    new["_proposer_tag"] = "stochastic"
...
def fetch_acc(db_path, exp_name, seed, epochs_min=1):
    import sqlite3
...
def evaluate_config(db_path, config_exp, seeds, epochs, cfg, label, data_dir, runs_dir,
                    db_name, max_procs, num_workers, ema_mode="none"):
    accs, to_run = {}, []
    for s in seeds:
        acc = fetch_acc(db_path, config_exp, s, epochs_min=epochs)
        if acc is None:
            to_run.append(s)
        else:
            accs[s] = acc
...
def main():
    ...
    db_path = os.path.abspath(os.path.join(args.runs_dir, args.db))
    conn = sqlite3.connect(db_path, timeout=60.0)
    ensure_tables(conn)
    ...
    inc_acc, inc_times = evaluate_config(
        db_path, inc_exp_screen, seeds, args.epochs, screen_inc_cfg,
    ...
    for it in range(1, args.budget + 1):
        ...
        print(f"[PGM-Ex6] iteration {it}/{args.budget}: screening phase (δ not consumed)")
        ...
        prop_acc_scr, prop_times_scr = evaluate_config(
            db_path, prop_exp_screen, seeds, args.epochs, screen_prop_cfg,
        ...
        # print(f"[iter {it:02d}][screen] per-seed Δ (pp): " + ", ".join([f"{s}:{(prop_acc_scr[s]-inc_acc[s]):+.2f}" for s in seeds]))
        # std_scr = statistics.pstdev(deltas_pp_scr) if len(deltas_pp_scr) > 1 else 0.0
        # print(f"[iter {it:02d}][screen] meanΔ={mean_pp_scr:+.2f}pp std={std_scr:.2f}")
        
        # mean_pp_scr = sum(deltas_pp_scr) / len(deltas_pp_scr)
        # snr = mean_pp_scr / (std_scr + 1e-6)
        # print(f"[iter {it:02d}][screen] SNR={snr:.2f}")

        # # EB lower bound (for logging on screen set)
        # y_scr = [math.tanh(d / args.rmax_pp) for d in deltas_pp_scr]
        # lb_scr, mu_scr = eb_lower_bound(y_scr, 0.5, R=1.0)  # logging only; δ=0.5 is arbitrary (no α spent in screening)
        # prop_mean_scr = sum(prop_acc_scr.values()) / len(seeds)
        # print(f"[iter {it:02d}][screen] (no α) μ={mu_scr:.4f} "
        ...
        print(f"[iter {it:02d}] Δ={mean_pp_scr:+.2f}pp | std={std_scr:.2f} | SNR={snr:.2f}")
        
        print(f"[iter {it:02d}] per-seed Δ: " +
              ", ".join([f"{s}:{(prop_acc_scr[s]-inc_acc[s]):+.2f}" for s in seeds]))
        
        # ---- EB bound ----
        # y_scr = [math.tanh(d / args.rmax_pp) for d in deltas_pp_scr]
        # lb_scr, mu_scr = eb_lower_bound(y_scr, 0.5, R=1.0)
        
        prop_mean_scr = sum(prop_acc_scr.values()) / len(seeds)
        
        print(f"[iter {it:02d}] inc={inc_mean:.2f}% → prop={prop_mean_scr:.2f}%")
        ...
        if not trigger_confirm:
            proposer_tag = prop_cfg.get("_proposer_tag", "multi-knob")
            ...
            notes = json.dumps({
                ...
                "lb_scr": None,
                "mu_scr": None,
                "time_min": {k: round(v/60.0,2) for k,v in (prop_times_scr or {}).items()}
            })
            ...
            """, (
                ...
                0, 0.0, len(seeds), float(args.rmax_pp),
                None, None,
        ...
        confirm_count += 1
        delta_conf = args.delta / (confirm_count * H_B)
        delta_used_total += delta_conf
        print(f"[CTHS] confirm #{confirm_count}: δ={delta_conf:.6f}")
        print(f"[CTHS] confirms={confirm_count}, δ_used_total={delta_used_total:.6f}")
        print(f"[CTHS] δ_remaining≈{args.delta - delta_used_total:.6f}")

        # ----- CONFIRM (expensive) -----
        seeds_conf = make_seed_set(seeds, args.confirm_n_seeds)
        ...
        inc_acc_conf, _ = evaluate_config(
            db_path, inc_exp_conf, seeds_conf, args.confirm_epochs, confirm_inc_cfg,
        ...
        prop_acc_conf, prop_times_conf = evaluate_config(
            db_path, prop_exp_conf, seeds_conf, args.confirm_epochs, confirm_prop_cfg,
        ...
        y_conf = [math.tanh((prop_acc_conf[s] - inc_acc_conf[s]) / args.rmax_pp) for s in seeds_conf]
        #lb_conf, mu_conf = hoeffding_lower_bound(y_conf, delta_t)
        lb_conf, mu_conf = eb_lower_bound(y_conf, delta_conf, R=1.0)
        ...
        elif args.policy == "naive_screen":
            decision = "ACCEPT" if mean_pp_scr > 0 else "REJECT"
        
        elif args.policy == "best_screen":
            decision = "ACCEPT"
        
        print(f"[iter {it:02d}][confirm-EB] μ={mu_conf:.4f} LB_conf={lb_conf:.4f} "
              f"inc_mean={inc_mean_conf:.2f}% prop_mean={prop_mean_conf:.2f}% decision={decision}")

        #proposer_tag = confirm_prop_cfg.get("_proposer_tag", prop_cfg.get("_proposer_tag", "multi-knob"))
        harmful_accept = (lb_conf <= 0) and (decision == "ACCEPT")
        proposer_tag = prop_cfg.get("_proposer_tag", "multi-knob")
        notes = json.dumps({
            "phase": "confirm",
            "changed": changed,
            ...
            "confirm_id": confirm_count,
            "time_min": {k: round(v/60.0,2) for k,v in (prop_times_conf or {}).items()},
            "harmful": harmful_accept
        })
        ...
        """, (
            ...
            float(delta_conf), len(seeds_conf), float(args.rmax_pp),
        ...
        if decision == "ACCEPT":
            ...
            # recompute new incumbent performance (IMPORTANT)
            screen_inc_cfg = apply_overrides(inc_cfg, SCREEN_OVERRIDES)
            inc_exp_screen = f"{args.exp_name}_inc_it{it}_screen_{cfg_hash(screen_inc_cfg)}"
        
            inc_acc, _ = evaluate_config(
                db_path, inc_exp_screen, seeds, args.epochs, screen_inc_cfg,
                f"new incumbent after iter {it}",
                data_dir=args.data_dir, runs_dir=args.runs_dir,
                db_name=os.path.basename(db_path),
                max_procs=args.max_procs, num_workers=args.num_workers,
                ema_mode=ema_mode
            ...
            print(f"[LONG] iter {it}: new incumbent mean={inc_mean:.2f}%")
            print(f"[ACCEPT] iter {it}: Δ={mean_pp_scr:+.2f}pp | SNR={snr:.2f} | LB={lb_conf:.4f}")
        
            if args.policy == "sgm" and not args.no_early_stop:
                break
