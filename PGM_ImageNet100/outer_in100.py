...
def main():
    ...
    for it in range(1, args.budget + 1):
        delta_t = args.delta / (it * H_B)
        #delta_t = min(0.05, args.delta / (it * H_B))
        ...
        inc_mean_conf  = sum(inc_acc_conf.values()) / len(seeds_conf)
        decision = "ACCEPT" if lb_conf >= 0 else "REJECT"
        #decision = "ACCEPT" if (lb_conf >= 0 or (mu_conf * args.rmax_pp) >= args.confirm_mean_pp) else "REJECT"
        ...
        if decision == "ACCEPT":
            ...
            print("[PGM-Ex6] ACCEPT achieved → early stopping.")
            break
